...
use super::{
    read_only::{refused, Dialect, ReadOnlyGuard},
    runs::Canceller, BatchSink, DbAdapter, DbError, DbResult, OpOutcome, QueryChunk, RunHandle,
};
use crate::api::{
    ColumnInfo, ConnGuard, FieldKeyTruncation, FieldShape, FilterOp, GridFilterCond, IndexInfo, QueryOp,
...
fn console_refusal(script: &str) -> Option<String> {
    let s = script.trim();
