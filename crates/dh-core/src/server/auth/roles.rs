...
//! "can manage roles" switch that lets an admin help.
//!
...
use crate::server::store::{audit_in, Store};
use sqlx::Row;
...
impl Store {
    ...
    pub async fn role_set(&self, actor: &AuthCtx, target_id: &str, new: ServerRole) -> Result<(), AccessError> {
        ...
        audit_in(&mut *tx, &actor.user_id, "server.role_changed", target_id, Some(&detail)).await?;
        tx.commit().await?;
        Ok(())
    }
    ...
    pub async fn manage_roles_set(&self, actor: &AuthCtx, target_id: &str, enabled: bool) -> Result<(), AccessError> {
        ...
        audit_in(&mut *tx, &actor.user_id, "server.manage_roles_changed", target_id, Some(detail)).await?;
        tx.commit().await?;
        Ok(())
    }
}
