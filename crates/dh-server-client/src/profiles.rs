...
//! keychain via the `keyring` crate, which this crate doesn't and shouldn't
...
mod tests {
    ...
    fn load_profiles_of_missing_file_is_empty() {
        let path = std::env::temp_dir().join(format!("dh-server-client-test-missing-{}.json", uuid::Uuid::new_v4()));
        assert_eq!(load_profiles(&path).unwrap(), Vec::new());
    }

    #[test]
    fn save_then_load_profiles_round_trips() {
        let path = std::env::temp_dir().join(format!("dh-server-client-test-profiles-{}.json", uuid::Uuid::new_v4()));
