//! Shared connection detail shapes, encrypted at rest on the server.
//! Passwords are AES-256-GCM sealed with the server master key and never
//! included in metadata listings. The `impl Store` CRUD, and
//! `AdapterParams` (the decrypted, never-serialized params handed to a
//! `DbAdapter`), are genuinely server only and live in `dh-server`'s own
//! `vault` module (spec 0012).
//!
...
//! MongoDB reuses the same common columns plus three Mongo-only ones
//! (`auth_db`/`srv`/`tls`, all optional/defaulted so Postgres rows are
//! unaffected).
