...
//! by the numbered files in `crates/dh-server/migrations/`.
//!
...
//!
//! `AuditEntry` and `now_ms` are shared with the desktop app (it never opens
//! a `Store`, but reuses the same time helper and, indirectly through its
//! own calls, the same audit shape) and live in `dh_server_client::store`
//! (spec 0012); re-exported here so the rest of this crate can keep writing
//! `crate::store::{now_ms, AuditEntry}`.

pub use dh_server_client::store::{now_ms, AuditEntry};
...
impl Store {
    ...
    pub async fn audit(
        &self,
        ctx: &dh_server_client::auth::AuthCtx,
        org_id: Option<&str>,
        action: &str,
        target: &str,
        detail: Option<&str>,
...
#[cfg(test)]
pub(crate) async fn test_user(store: &Store, email: &str, role: dh_server_client::auth::ServerRole) -> dh_server_client::auth::User {
