...
//! verified. The parsers below are pure so they test without a network, but
//! only the server ever calls them (the desktop app never sees a raw
//! provider response), so they stay here rather than in `dh-server-client`.

use dh_server_client::auth::{ProfileOutcome, VerifiedProfile};
...
/// re-checks on the callback (see `router`'s auth routes).
