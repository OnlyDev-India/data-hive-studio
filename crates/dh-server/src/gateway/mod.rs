...
//! the connection's `kind` (see [`crate::vault::AdapterParams`]) so
...
//! them — see `orgs.rs`/`grants.rs`. The one shared shape (`ConnWithAccess`)
//! lives in `dh_server_client::gateway` (spec 0012).
...
use dh_core::db::{DbAdapter, MongoAdapter, PgAdapter};
use dh_server_client::auth::AuthCtx;
use dh_server_client::grants::DataAccess;
...
impl Gateway {
    ...
    pub(super) async fn effective_access(
        ...
        let meta = self.store.conn_get(conn_id).await?.ok_or(crate::vault::ERR_NOT_FOUND)?;
