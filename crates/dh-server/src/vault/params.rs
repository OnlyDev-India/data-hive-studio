    ...
    pub async fn conn_secret_params(&self, id: &str) -> Result<AdapterParams, String> {
        ...
        let ssh = match &row.ssh_host {
            ...
            Some(host) => {
                ...
                Some(dh_core::ssh_tunnel::SshConfig {
                    host: host.clone(),
        ...
        Ok(match kind_from_str(&row.kind) {
            ...
            DbKind::Mongodb | DbKind::DocumentDb => AdapterParams::Mongodb(dh_core::db::MongoParams {
                host: row.host,
            ...
            _ => AdapterParams::Postgres(dh_core::db::PgParams {
                host: row.host,
                port: row.port as u16,
                user: row.user,
                password,
