...
//! (`/auth/...`, which establish or renew a session). Org membership/role is
//! enforced per call — see `gateway.rs`/`orgs.rs`. The request body types
//! shared with the desktop client live in `dh_server_client::client`
//! (spec 0012).
