...
pub(super) async fn auth_providers() -> Json<Vec<&'static str>> {
    Json(
        ["google", "github"]
            .into_iter()
            .filter(|p| crate::auth::provider_config(p).is_some())
            .collect(),
    )
}
...
pub(super) async fn auth_start(Path(provider): Path<String>, Query(q): Query<StartQuery>) -> Response {
    let Some(cfg) = crate::auth::provider_config(&provider) else {
    ...
    let Some(url) =
        crate::auth::authorize_url(&provider, &cfg, &redirect_uri, &csrf)
    else {
...
pub(super) async fn auth_callback(
    ...
    let redirect_uri = format!("{}/auth/{}/callback", public_base_url(), provider);
    let outcome = match crate::auth::exchange_code(&provider, &cfg, &q.code, &redirect_uri).await {
        Ok(o) => o,
...
pub(super) struct MeOrg {
    #[serde(flatten)]
    pub(super) org: dh_server_client::orgs::Organization,
